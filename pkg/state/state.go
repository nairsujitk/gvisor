...
// 	m.Save(&x.A) // Field is A.
// 	m.Save(&x.B) // Field is B.
// }
//
// func (x *X) StateLoad(m Source) {
// 	m.Load(&x.A) // Field is A.
// 	m.Load(&x.B) // Field is B.
// }
func (s Sink) Save(objPtr interface{}) {
	s.internal.save(reflect.ValueOf(objPtr).Elem())
}
...
//
// func (x *X) StateLoad(m Source) {
//	m.LoadValue(new(int64), func(x interface{}) {
//		x.A = P.Foo(x.(int64))
//	})
// }
func (s Sink) SaveValue(obj interface{}) {
	s.internal.save(reflect.ValueOf(obj))
}
...
type Type interface {
	// StateTypeInfo returns information about the type.
	//
	// This will be called at most once per serialization.
	StateTypeInfo() TypeInfo
}
...
func (s Source) Load(objPtr interface{}) {
	s.internal.load(reflect.ValueOf(objPtr), false, nil)
}
...
//
// See Sink.Save for an example.
func (s Source) LoadWait(objPtr interface{}) {
	s.internal.load(reflect.ValueOf(objPtr), true, nil)
}

// LoadValue loads the given object value from the map.
//
// See Sink.SaveValue for an example.
func (s Source) LoadValue(objPtr interface{}, fn func(interface{})) {
	o := reflect.ValueOf(objPtr)
	s.internal.load(o, true, func() { fn(o.Elem().Interface()) })
}
...
// maybeFailf checks for an existing StateErr, otherwise wraps it.
func maybeFailf(err error, fmtStr string, v ...interface{}) {
